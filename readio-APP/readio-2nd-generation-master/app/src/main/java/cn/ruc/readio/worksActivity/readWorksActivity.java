...
public class readWorksActivity extends Activity implements ViewSwitcher.ViewFactory, View.OnTouchListener {
    ...
    private ConstraintLayout orderBar = null;
    private User user;
    ...
    private float downX;
    private ArrayList<Drawable> imgDrables = new ArrayList<>();
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        orderBar = (ConstraintLayout) findViewById(R.id.photo_orderBar);
            ...
            public void onResponse(Call call, Response response) throws IOException {
                try {
                    if (response.code() == 200) {
                        ...
                        //根据fileId，获取图片
                        JSONArray picArray = data.getJSONArray("picArray");
                        for (int i = 0; i < picArray.length(); i++) {
                            String fileId = picArray.getString(i);
                            Bitmap picData = Tools.getImageBitmapFromServerSyn(fileId);
                            Bitmap scaledPicData = Tools.ZoomBitmapImageMatchParent(readWorksActivity.this, picData);
                            imgDrables.add(new BitmapDrawable(scaledPicData));
                        }
                        runOnUiThread(new Runnable() {
                            @Override
                            public void run() {
                                try {
                                    if (imgDrables.size() >= 1) {
                                        mImageSwitcher.setVisibility(View.VISIBLE);
                                        orderBar.setVisibility(View.VISIBLE);
                                        mImageSwitcher.setImageDrawable(imgDrables.get(currentPosition));
                                        photoOrder.setText(toString().valueOf(currentPosition + 1) + " / " + toString().valueOf(imgDrables.size()));
                                    } 
...
//                FrameLayout.LayoutParams.WRAP_CONTENT, ImageSwitcher.LayoutParams.WRAP_CONTENT));
...
//                isDowned = true;
                ...
                if (downX > mid_width) {
                    if (currentPosition < imgDrables.size() - 1) {
                        currentPosition++;
                        mImageSwitcher.setImageDrawable(imgDrables.get(currentPosition));
